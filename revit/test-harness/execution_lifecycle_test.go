...
`
	...
	if started.Status != "pending" && started.Status != "running" {
		t.Fatalf("a script outliving timeout_ms must return pending/running, got %q (execution_id: %s, return_value: %s)", started.Status, started.ExecutionID, started.ReturnValue)
	}
	...
	if polled.Status != "pending" && polled.Status != "running" {
		t.Fatalf("polling a live run must report pending/running, got %q (return_value: %s)", polled.Status, polled.ReturnValue)
	}
	...
	if after.Status != "success" {
		t.Fatalf("instance still not usable after a cancelled run: status=%q return_value=%s", after.Status, after.ReturnValue)
	}
}
