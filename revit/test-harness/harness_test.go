...
func TestDenylistRejectsOwnTransaction(t *testing.T) {
	...
	for _, tc := range []struct{ name, member, script string }{
		...
		// A SubTransaction is PERMITTED, but only as a `using` resource (#146 Phase 1): the bare
		// construction is refused with the same code and a remedy naming the `using` shape.
		{"SubTransactionOutsideUsing", "Autodesk.Revit.DB.SubTransaction", `var st = new Autodesk.Revit.DB.SubTransaction(Document); st.Start(); st.Commit(); return "ran";`},
	} {
