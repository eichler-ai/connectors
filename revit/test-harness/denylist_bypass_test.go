...
return "opened an unmanaged transaction";`,
	...
	if out.Status != "success" {
		t.Errorf("instance unusable after the rejections: status=%q return_value=%s", out.Status, out.ReturnValue)
	}
}
...
return "captured an adapter through a callback";`
	...
	out := runScript(t, c, instanceID, documentID, `return Document.Title;`)
	if out.Status != "success" {
		t.Errorf("instance unusable after the rejection: status=%q return_value=%s", out.Status, out.ReturnValue)
	}
}
