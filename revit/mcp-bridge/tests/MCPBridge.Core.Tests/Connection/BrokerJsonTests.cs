...
public class BrokerJsonTests
{
    ...
    public void Parse_ReadsAllFields()
    {
        ...
        Assert.Equal(new DateTimeOffset(2026, 8, 25, 10, 0, 0, TimeSpan.Zero), brokerJson.StartTime);
        Assert.Equal("s3cr3t-token", brokerJson.Token);
    }

    [Theory]
    [InlineData("""{"pid":1,"start_time":"2026-01-01T00:00:00Z","token":"t"}""")]
    [InlineData("""{"port":1,"start_time":"2026-01-01T00:00:00Z","token":"t"}""")]
    [InlineData("""{"port":1,"pid":1,"token":"t"}""")]
    [InlineData("""{"port":1,"pid":1,"start_time":"2026-01-01T00:00:00Z"}""")]
    ...
    {
        var json = """{"port":1,"pid":1,"start_time":"2026-01-01T00:00:00Z","token":""}""";
        Assert.Throws<BrokerJsonParseException>(() => BrokerJson.Parse(json));
    }
}
