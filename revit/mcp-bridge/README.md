...
- `src/MCPBridge.AddIn/` — `IExternalApplication` entry point + `.addin` manifest: connection loop with runtime reconnect/mode switch (`BridgeHost.cs`), local/remote topology resolution (`MCPBridgeApplication.BuildDiscoveryOptions` over Core's `BrokerModeResolver`: `bridge-config.json` → `MCPBRIDGE_BROKER_MODE` env → Local), and the ribbon's Status / Reconnect / "MCP Server: Local|REMOTE" buttons (UI text says "MCP Server", never "broker"). Multi-targets `net10.0-windows`/`net8.0-windows` (Revit 2027/2025, PRD §11).
