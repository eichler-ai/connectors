...
func submitHowTo(deps HowToDeps, in SubmitHowToIn) SubmitHowToOut {
	...
	// A first-class notice, not only guidance prose: with no token the issue is
	// NOT filed, and confirm_submission succeeding otherwise reads as "done".
	// Four real submissions were reported as "confirmed to review queue" when
	// they had only reached this outbox (they never left the machine), so the
	// not-filed state has to surface the way every other not-done state on this
	// path does -- as a diagnostic the agent cannot skim past.
	...
	out.Guidance = "The scrubbed document is what will leave this machine -- read it with the user. The connector did NOT file the issue and nothing has reached the review queue yet (no REVIT_MCP_GITHUB_TOKEN is set), so file it yourself with the GitHub tool you have: " +
