...
type Env struct {
	...
	// session sidecar as verified.jsonl, outbox/ beneath it).
	LocalDir string
...
const (
	...
	// OutboxDir is the subdirectory holding prepared submissions.
	OutboxDir = "outbox"
...
func Save(env Env, sub Submission) (*Saved, error) {
	...
	saved := &Saved{Doc: d, LocalPath: path, Replaced: statErr == nil}
		...
		if at, ok := env.SessionSucceeded(ScriptSHA256(d.Script)); ok {
			...
			if raw, err := json.Marshal(st); err == nil {
				if _, err := ValidateStamp(raw); err == nil {
					f, err := os.OpenFile(filepath.Join(env.LocalDir, SessionSidecarName), os.O_APPEND|os.O_CREATE|os.O_WRONLY, 0o644)
					if err == nil {
...
// Residue names a scrub pattern that still matches after scrubbing: the
...
var scrubPatterns = []struct {
	...
	{"unc-path", regexp.MustCompile(`\\\\[A-Za-z0-9._$-]+\\[^\s"'\)\]]*`), "<path>"},
	{"windows-path", regexp.MustCompile(`\b[A-Za-z]:\\[^\s"'\)\]]*`), "<path>"},
	{"file-url", regexp.MustCompile(`file://[^\s"'\)\]]*`), "<path>"},
	{"posix-path", regexp.MustCompile(`(?:^|[\s"'(=])(/(?:Users|home|tmp|var|private|Volumes|mnt|opt)/[^\s"'\)\]]*)`), "<path>"},
	{"env-path", regexp.MustCompile(`%[A-Za-z_]+%\\[^\s"'\)\]]*`), "<path>"},
...
// returns the scrubbed copy plus any residue the patterns could not remove.
// Document titles, the machine name and the user name are removed as words,
// case-insensitively, before the generic patterns run.
func Scrub(d *Document, env Env) (*Document, []Residue) {
	out := *d
	names := []struct{ word, repl string }{}
	for _, t := range env.DocumentTitles {
		if t = strings.TrimSpace(t); len(t) >= 3 {
			names = append(names, struct{ word, repl string }{t, "<document>"})
		}
	}
	if h := strings.TrimSpace(env.Hostname); len(h) >= 3 {
		names = append(names, struct{ word, repl string }{h, "<host>"})
	}
	if u := strings.TrimSpace(env.Username); len(u) >= 3 {
		names = append(names, struct{ word, repl string }{u, "<user>"})
	}
	// Longest names first so a title containing the user name is replaced whole.
	sort.Slice(names, func(i, j int) bool { return len(names[i].word) > len(names[j].word) })
	var residue []Residue
	scrub := func(field, text string) string {
		...
		for _, n := range names {
			re := regexp.MustCompile(`(?i)` + regexp.QuoteMeta(n.word))
			text = re.ReplaceAllString(text, n.repl)
		}
		for _, p := range scrubPatterns {
			text = p.re.ReplaceAllStringFunc(text, func(m string) string {
				// keep the leading delimiter the posix pattern consumed
				if p.kind == "posix-path" {
					lead := ""
					if m[0] != '/' {
						lead = m[:1]
					}
	...
	// submission provenance on acceptance. Strip anything path-like there too.
...
type Prepared struct {
	...
	GhCommand string // the command the agent runs to file it
	Labels    []string
}
...
func Prepare(env Env, saved *Saved, changeNote string) (*Prepared, error) {
	...
	dir := filepath.Join(env.LocalDir, OutboxDir)
	...
	gh := fmt.Sprintf("gh issue create --repo %s --template %s --title %q --body-file %q", env.RepoSlug, IssueTemplate, title, bodyPath)
	return &Prepared{Scrubbed: scrubbed, OutboxDoc: docPath, BodyPath: bodyPath, IssueURL: issueURL, GhCommand: gh, Labels: labels}, nil
}
