...
**Real Revit API access from scripts (Phase 3 precursor)** *(researched, not yet designed)* — `ScriptGlobals.Document` is deliberately narrow (`IScriptDocument`, `Title` only, §06); passing it where a real Revit API type is expected fails to compile (`CS1503`, confirmed live). Live research this session found the actual API IS reachable today, just not through that sanctioned seam: reflecting into `RevitDocumentAdapter`'s private `_document` field reaches the real `Autodesk.Revit.DB.Document`, and real writes through it succeed — `Level.Create`, confirmed live, with no competing-transaction failure — because `TransactionScriptExecutor`'s existing ambient transaction already covers any write made through the real `Document`, regardless of how a script obtained the reference. That settles what was assumed to be the hard design question (a new transaction-ownership scheme): none is needed, since the thing that was actually closed off twice after review (§06, `IScriptDocument`'s own doc comment) was scripts opening their *own* competing `Transaction`/`TransactionGroup` via `IDocumentAdapter.CreateTransaction`/`CreateTransactionGroup` — not reads or writes through the `Document` object itself. **Still undesigned**: how much of `Autodesk.Revit.DB`/`UI` to expose (real `Document` vs. a wider adapter), and — the harder remaining question — which specific `Document` methods to explicitly deny even though raw exposure would technically allow them (`Close`, `SaveAs`, worksharing sync/relinquish, print). That denylist can't be enumerated against the ~1,700-type surface up front; expect it to grow from real use, not be fully specified before Phase 3 ships. `get_skills`' live guide (`revit/mcp-server/internal/mcpserver/skill.md`) documents the reflection technique as a today-only workaround, explicitly not the intended long-term design.
...
4. **Validation corpus.** *(partial)* The tutorial-workflow corpus itself (15–20 cases, beginner through advanced) hasn't been built — it needs a *sanctioned* way for scripts to reach real Revit API elements, which doesn't exist yet (the only way there today is an unsupported reflection workaround, not something a corpus should depend on — see §14's "Real Revit API access from scripts (Phase 3 precursor)" finding). What exists instead: `revit/test-harness/`, a live MCP-over-stdio test harness (one case so far, `TestCreateLevel`) covering what's testable without that — registration, discovery disambiguation, file exchange, error shapes — a genuine regression suite, just not this phase's originally-scoped corpus.
