...
- **Status:** phases 1–3 shipped (core loop, dialog suppression + multi-instance, API discovery + file exchange), phase 6 partial (Revit 2025 support) — see §15 for per-phase status
...
Instance registration: on connect, each add-in sends a `register` notification with instance ID (GUID, generated once per Revit process at `OnStartup` — stable for that process's lifetime, independent of any particular broker connection), PID, Revit version, and the list of currently open documents. **That list is a snapshot taken at connect time, not a live feed** — an earlier version of this line said "(updated on open/close)", which the implementation has never done: `RegisterMessage` is constructed only on connect (`BridgeHost`), and there is no `DocumentOpened`/`DocumentClosed` subscription anywhere in the add-in. A document opened after the connect therefore does not appear in `list_instances` until something forces a reconnect. Live push is a plausible enhancement; it is not what ships today, and the gap is worth knowing because an empty `documents[]` reads like a bug when it is usually just this. The broker maintains this as live state and also mirrors it to a local `instances.json` so the broker itself is restartable without losing visibility (Revit instances just reconnect).
...
The earlier finding held up: no new transaction-ownership scheme was needed. `TransactionScriptExecutor`'s ambient `Transaction`/`TransactionGroup` already covers any write made through the real `Document` however a script obtained the reference. What the deleted interfaces actually guarded — `IDocumentAdapter.CreateTransaction`/`CreateTransactionGroup` — turned out to be the wrong target: those are *our own adapter methods*, not Revit API, and unreachable from a script once `Document` is the real type. The real invariant is a script constructing its own `Autodesk.Revit.DB.Transaction`/`TransactionGroup`/`SubTransaction`, and it is enforced by **`ScriptApiDenylist`** (`revit/mcp-bridge/src/MCPBridge.Core/Execution/ScriptApiDenylist.cs`): a semantic walk over the bound Roslyn compilation, run from `RoslynScriptRunner.GetOrCompile` at the same point as the existing top-level-`await` rejection, throwing `ScriptApiDenylistViolationException` (code `script-api-denied`). Because the ambient transaction is opened *before* compilation, a rejection unwinds through the identical rollback path as any compile error — no new failure handling.

Final denylist shape, matching this section's original starting list: constructing `Transaction`/`TransactionGroup`/`SubTransaction`; `Document.Close`/`Save`/`SaveAs`/`SynchronizeWithCentral`/`Print`/`PrintToFile`; `WorksharingUtils.RelinquishOwnership`. (Note this section previously wrote `SynchronizeWithCentralDocument`; the actual 2027 member is `Document.SynchronizeWithCentral`, confirmed via `describe_function`.) Keyed on *(containing type, member)*, not bare name, so ordinary `System.IO.Stream.Close` is unaffected. As predicted, this is a starting point expected to grow from real use rather than an exhaustive policy over the ~1,700-type surface, and it is a guard against accidental damage, not a sandbox — reflection can still route around it, which is accepted.
...
4. **Validation corpus.** *(unblocked, not yet built)* The tutorial-workflow corpus itself (15–20 cases, beginner through advanced) hasn't been built. What blocked it — no *sanctioned* way for a script to reach real Revit API elements, only an unsupported reflection workaround a corpus should never depend on — is resolved: see §14's "Real Revit API access from scripts", now shipped. Building the corpus on `ScriptGlobals.Document` is a real option from here. What exists so far: `revit/test-harness/`, a live MCP-over-stdio test harness covering registration, discovery disambiguation, file exchange, error shapes, the sanctioned globals, and the `ScriptApiDenylist` rejections — a genuine regression suite, just not this phase's originally-scoped corpus.
5. **Signed distribution.** *(not started)* Code-signing certificate, `signtool` build step, installer, GitHub Releases distribution — closes the "untrusted publisher" friction ahead of any wider testing. `install.ps1`/`install-mac.sh` already exist and work against a local test payload (`-LocalPackagePath`); what's missing is the release pipeline itself producing real signed artifacts for them to fetch.
