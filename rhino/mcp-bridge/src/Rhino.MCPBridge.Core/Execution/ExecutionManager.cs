...
public sealed class ExecutionManager
{
    ...
    /// <summary>Running -&gt; Pending, when the run's command could not be started (Rhino inside another
    /// command). The launcher retries; a cancellation arriving meanwhile still wins via the token.</summary>
    public void MarkPendingAgain(string executionId) =>
        Transition(executionId, "mark-pending-again", record => record.MarkPendingAgain(), clearActive: false);
    ...
    public void CheckGraceExpiry(DateTimeOffset now)
    {
        lock (_lock)
        {
            ...
            var diagnostic = DiagnosticRecord.Create(
                ...
                remedy: new[] { "Restart Rhino to recover this instance; a fresh instance_id will be issued on reconnect." });
    ...
    private static DiagnosticRecord UnrecoverableDiagnostic() => DiagnosticRecord.Create(
        ...
        DiagnosticSource.Execution,
        "this Rhino instance is unrecoverable after a cancellation that did not complete within its grace period.",
        detail: null,
        remedy: new[] { "Restart Rhino to recover this instance; a fresh instance_id will be issued on reconnect." });
}
