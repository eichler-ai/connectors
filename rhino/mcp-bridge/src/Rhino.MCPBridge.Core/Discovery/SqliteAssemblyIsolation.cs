...
/// <summary>
/// Same fix as <see cref="Rhino.MCPBridge.Core.Execution.RoslynAssemblyIsolation"/>, for the same underlying
/// reason: Revit's AddInLoader loads this add-in via <c>Assembly.LoadFrom</c>, not a deps.json-driven
/// host, so Rhino.MCPBridge.Core.dll's own PackageReference dependencies (Microsoft.Data.Sqlite,
/// SQLitePCLRaw.*) are NOT automatically found via default assembly-resolution probing even though they
/// sit physically right next to Rhino.MCPBridge.Core.dll in the Addins folder -- confirmed live: a bare
