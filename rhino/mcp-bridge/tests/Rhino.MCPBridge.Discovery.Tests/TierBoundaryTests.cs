...
public class TierBoundaryTests
{
    ...
    public void NoTierTwoRowIsEmittedWithZeroRelevance()
    {
        ...
        using var cache = RealRhinoCorpus.Build();
    ...
    public void AWeakButNonZeroMatchStillReachesTierTwo()
    {
        ...
        var kilonewtons = cache.Search("millimeters", namespaceFilter: null)
            .Where(r => r.Member.Name.Contains("Millimeter", StringComparison.Ordinal))
            .ToList();

        Assert.True(kilonewtons.Count > 0, "no Millimeter member matched at all; the fixture assumption is wrong");
        Assert.True(
