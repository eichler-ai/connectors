...
public class WireEnumNameConverterTests
{
    ...
    public void Read_NonStringToken_ThrowsJsonException(string json)
    {
        ...
        var ex = Record.Exception(() => JsonSerializer.Deserialize<DiagnosticSeverity>(json));
    ...
    {
        var ex = Assert.Throws<JsonException>(() => JsonSerializer.Deserialize<DiagnosticSeverity>(json));

        Assert.Contains("DiagnosticSeverity", ex.Message);
    }
    ...
    public void Write_UndefinedValueCastIntoEnum_ThrowsRatherThanEmittingANumber()
    {
        ...
        var undefined = (DiagnosticSeverity)7;
