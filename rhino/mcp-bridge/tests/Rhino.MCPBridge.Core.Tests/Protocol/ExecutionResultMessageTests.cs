...
public class ExecutionResultMessageTests
{
    ...
    {
        // The Go side's execution.Result reads exactly these names (PRD §07); keep the two in step.
        var record = ExecutionRecord.CreatePending("exec-1", "1+1", 600_000, Now);
        record.MarkRunning(Now);
        record.MarkCompleted(Now, "ok", null, Array.Empty<DiagnosticRecord>(), mutations: new MutationReport
        {
            NetAdded = 2, NetModified = 1, NetDeleted = 0,
            ByObjectType = new Dictionary<string, MutationTally> { ["Brep"] = new() { Added = 2 }, ["Curve"] = new() { Modified = 1 } },
            ByLayer = new Dictionary<string, MutationTally> { ["Default"] = new() { Added = 2, Modified = 1 } },
        });

        var json = ExecutionResultMessage.FromRecord(Id, record);

        Assert.Contains("\"mutations\":{\"net_added\":2,\"net_modified\":1,\"net_deleted\":0,\"by_object_type\":{\"Brep\":{\"added\":2,\"modified\":0,\"deleted\":0},\"Curve\":{\"added\":0,\"modified\":1,\"deleted\":0}},\"by_layer\":{\"Default\":{\"added\":2,\"modified\":1,\"deleted\":0}}}", json);
