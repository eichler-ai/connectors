...
public class ReturnValueFormatterTests
{
    ...
    /// <summary>
    /// The sibling half of the rule above, and the one that keeps a returned Rhino Element from being
    ...
    /// lose every other value in the graph to one degenerate number. Rhino geometry produces both
    ...
    /// <summary>
    /// A type with no ToString() override stands in for a Rhino API object (which a tier-1 assembly cannot
    /// reference -- RhinoAPI.dll is mixed-mode and unloadable here, see the skill's testing strategy).
    ...
    /// COMPLETED run into an unhandled exception on Rhino's UI thread.
    /// </summary>
    [Fact]
    ...
    /// runs on Rhino's UI thread. Enumerating it to completion would wedge Rhino -- strictly worse than
    ...
    public async Task InfiniteSequence_IsBoundedRatherThanEnumeratedToCompletion()
    {
        ...
        Assert.True(finished, "formatting an endless sequence must terminate; on Rhino's UI thread this hangs the whole application");
    ...
    /// inside every count limit can still be slow per item -- a Rhino property getter doing real work is
    /// the live case -- and this runs on the UI thread after the run is already complete, past any
    /// max_duration_ms cancellation. 300 items x 25 ms would be 7.5 seconds of wedged Rhino.
    /// </summary>
    [Fact]
    ...
    /// The Rhino metadata references are required for ScriptGlobals' own members to bind at all (see
    ...
    private static object RunScriptReturning(string script)
    {
        ...
        var globals = TestGlobals.Create();
    ...
    /// <summary>Stands in for a collection whose per-item cost is real work -- a Rhino property getter.</summary>
