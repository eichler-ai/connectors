...
// The JSON Schemas are the interchange contract; this file embeds them so the
// broker validates with exactly the published text.
//
...
// cross-field alike, so an author can fix them in one pass.
...
// crossFieldProblems are the rules howto-schema.json names as "enforced by the
// Go validator": absorbs excludes own id; contributors[].rev <= rev;
// created_at/updated_at present and ordered; hit/miss query shape.
func crossFieldProblems(d *Document) []string {
	var p []string
	if slices.Contains(d.Absorbs, d.ID) {
