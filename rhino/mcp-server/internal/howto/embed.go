...
// The corpus ships INSIDE the broker: one JSON file per lineage under corpus/
// (the latest revision only; git history is the audit trail) and the sweep's
// sidecar of verification stamps. A file per document, rather than the
// one-line-per-lineage JSONL the loader also reads, because these are edited
// by hand and reviewed as diffs — a 6 KB script on one JSONL line is
// unreviewable. LoadCorpus still gets JSONL: the files are joined into it at
// load, so the one loader and its checks serve both.
//
//go:embed corpus/*.json corpus/verified.jsonl
var corpusFS embed.FS

// CorpusDir is where the files live in the repository, for messages.
const CorpusDir = "rhino/mcp-server/internal/howto/corpus"

// Version identifies a corpus build: how many documents, a content hash
// (documents only, in id order — stamps do not change it), and the Rhino
...
func (v Version) String() string {
	...
	if len(v.VerifiedOn) > 0 {
		s += ", verified on Rhino " + strings.Join(v.VerifiedOn, ", ")
	}
	return s
}
...
func loadEmbedded(fsys fs.FS) (*Corpus, []Stamp, Version, error) {
	...
	for _, st := range stamps {
		if st.Status == StampPassed && !seen[st.RhinoVersion] {
			seen[st.RhinoVersion] = true
			ver.VerifiedOn = append(ver.VerifiedOn, st.RhinoVersion)
		}
	}
