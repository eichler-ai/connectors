...
type ExecuteScriptIn struct {
	...
	GrasshopperDocumentID   string `json:"gh_document_id,omitempty" jsonschema:"optional gh_document_id (from list_instances' gh_documents) of the Grasshopper definition to bind: the script's ghdoc (Python) / GrasshopperDocument (C#) global becomes that GH_Document. Omit to leave it null. In C# it is typed object -- cast it: (Grasshopper.Kernel.GH_Document)GrasshopperDocument. An id matching no open definition fails with grasshopper-document-not-found (Grasshopper must be open with that definition loaded)"`
