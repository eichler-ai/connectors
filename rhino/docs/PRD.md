...
**A freshly installed plug-in is on disk but not loaded.** Rhino (and Grasshopper) load new packages only at startup, and there is no supported way to hot-load a `.gha` into a running Grasshopper, so every `install_plugin` result says a Rhino restart is required to use it. Putting a plug-in "into use" is therefore a companion tool, **`restart_rhino(instance_id, confirm_lifecycle_actions, discard_unsaved?)`** — gated like the others. It reads a read-only `restart_snapshot` from the bridge (every open Rhino document and Grasshopper definition with its path and modified flag), **refuses** (`blocked`) when anything is unsaved unless `discard_unsaved` is set, and on confirm quits the Rhino process and relaunches it, reopening the **saved Rhino documents** by path (Grasshopper definitions and unsaved/untitled documents are not reopened; macOS only in v1). The in-process `Yak.Core` API exists but is undocumented and unsupported, so the supported CLI is used instead; this keeps the bridge thin (the server, not the plug-in, does the work — the bridge only reports the snapshot; the server drives the restart).
...
Rhino has a first-party package manager, **yak**, with a public server McNeel runs and the `_PackageManager` command inside Rhino on both platforms; the most popular incumbent ships through it. A yak package is a zip with a manifest; it can carry the plug-in for both platforms and arbitrary files alongside. The old `.rhi` / RhinoInstallerEngine path is **obsolete as of Rhino 7** (McNeel), so yak is not merely first-party, it is the only forward path. Rhino does not require Authenticode or notarization to load a plug-in, though macOS Gatekeeper does for the *server* binary launched by Claude.

**Yak updates on restart, automatically.** Rhino 8 checks each installed package's source on launch and installs a newer version before load — no user action, no in-app "Update available" click, no custom updater (this corrects the earlier framing of updates as a manual `_PackageManager` step). This is the whole robustness argument for yak: the update mechanism is McNeel's, not ours. There is a global opt-out in the Package Manager but no documented per-package one. A companion binary carried in the same package (below) rides this signal transitively — updating the plug-in updates the binary.

**Distribution channel — public server, with a private fallback.** v1 publishes to the public server (`yak.rhino3d.com`): zero infrastructure, it is where users already search, auto-update is free, and it matches the connector's public-distribution posture (Revit's `irm | iex`). The alternative, if public discoverability is ever unwanted, is a **Custom Package Repository** — a folder or an HTTP source added to `Rhino.Options.PackageManager.Sources` — which reuses all of yak's install/update machinery but costs the user a one-time Sources config step and us the infra. Public unless a reason to go private appears.

Proposed: **one yak package carries the plug-in and both platforms' server binaries**; a Rhino command `MCPBridgeRegister` writes the MCP client registration (`claude mcp add` when the CLI is present, else the JSON snippet shown in the Status panel) pointing at the server binary the package installed. The plug-in re-registers on load if the server path moved, and `update_connector` becomes a check plus a pointer at the package manager rather than an installer of its own — the shim/versioned-folder machinery Revit needed (issue #211) is unnecessary because yak already installs beside the running version and Rhino loads the new one at next start. The server binary's macOS notarization is the one signing cost, and it is deferred exactly as Revit's CA certificate is, with the "unidentified developer" prompt documented until then. `yak build` / `yak install <file>.yak` / `yak uninstall` are **verified** as the scriptable local install path on macOS (the Mac's `_-PlugInManager` has no load-from-path option, and Rhino only scans a package folder that carries yak's `manifest.txt` marker).

**Package size — de-embed the models for download speed, not for yak's sake.** Each platform's server binary is ~80 MB, ~52 MB of which is the two embedded search models (potion-base + the ms-marco reranker). Whether yak accepts a payload that large was open question §17 #9; the phase-7 Windows spike (`spikes/phase-7-windows-distribution.md`, 2026-09-13) **answered it — yes, even un-slimmed**: `yak build` packaged the fat 82 MB `mcp-server.exe` beside the plug-in into a 62 MB `.yak`, `yak install` unpacked it intact, and the plug-in loaded clean with the extra binary in the folder. So slimming is no longer a correctness gate — but it is still wanted: fetch the models on first run (the same B2 work on Revit) drops each binary to ~28 MB and the package to ~a third, a much faster Package-Manager download. Worth doing, not blocking.

**How the companion Go binary rides — plan A, with B as fallback.** *(A)* The yak package carries the plug-in and both slimmed binaries; on load the plug-in extracts its platform's binary and runs `MCPBridgeRegister`. One artifact, one version, and yak's auto-update carries the binary for free — the simplest to operate and the default. *(B)* The yak package carries only the plug-in, which fetches its platform's binary from the GitHub release on first load. Keeps the package tiny but reintroduces a second update path and the fetch code (which Revit already has). A is the plan; B is cheap insurance if size or yak's payload rules force it.
...
9. ~~yak package size and non-plug-in payload acceptance (§15)~~ **Verified on Windows 2026-09-13** (`spikes/phase-7-windows-distribution.md`): a `net8.0` plug-in plus the fat 82 MB server binary in one yak package builds (62 MB `.yak`), `yak install`s, unpacks the binary intact, and the plug-in loads clean; the yak-installed server then dials into the plug-in and `list_instances` returns the Windows instance. `claude mcp add` → native `claude.exe` → `Connected`, and the unsigned Go exe runs with Defender RTP on and no quarantine. Plan A viable, unmodified. Model-slimming is a download-speed nicety, not a gate. Still open: the `MCPBridgeRegister` command itself, `yak push` to the public server, auto-update-from-source, and the Mac half (notarization).
...
7. **Distribution.** The yak package, `MCPBridgeRegister`, `update_connector` as a check, macOS notarization deferred. **Windows first** — it is the first install target and carried the load risk (a `net8.0` plug-in must load on a Windows Rhino 8 SR, §14). The manual end-to-end is **proven** (`spikes/phase-7-windows-distribution.md`, 2026-09-13): combined yak package installs, plug-in loads clean, the packaged server dials in, `claude mcp add` connects, unsigned exe clears Defender. What remains is productizing it: the `MCPBridgeRegister` command (shells out to `claude mcp add`, else shows the JSON), a CI job that builds both platforms' binaries and `yak push`es, model-slimming for download speed, and auto-update-from-source. Success: a fresh Windows machine, then a fresh Mac, each install from `_PackageManager` and register with Claude with no manual steps beyond one command.
