...
RhinoCommon must be called on Rhino's main thread (**verified**: every surveyed plug-in marshals with `RhinoApp.InvokeOnUiThread`, and script-sync's own comments say the script editor refuses to run from a worker thread). The TCP listener and both script hosts' compilation run off-thread; every API touch is marshalled with `RhinoApp.InvokeOnUiThread`, which queues onto the main thread's message loop. The plug-in reports `pending` until the queued work actually starts and `running` once it has, exactly as Revit §06 distinguishes them, because the main thread may be inside a command, a modal dialog, or a Grasshopper solve.
...
| stdout | `RunContext` output capture, plus `RhinoApp.CommandWindowCaptureEnabled` for lines Rhino itself prints | AsyncLocal console capture as Revit, plus the same command-window capture |
...
Rhino has no transactions. Its document is always modifiable, and its unit of undo is the **undo record**: `RhinoDoc.BeginUndoRecord(description)` returns a serial number, every change until `EndUndoRecord(serial)` is one entry in the Undo stack, and `RhinoDoc.Undo()` reverts the top entry. That is enough to keep Revit's two user-facing guarantees, with one honest weakening:

- **One run, one Undo entry.** The executor opens an undo record before the script runs, named `MCP: <label>` or a name derived from the mutation report (`MCP: 12 objects added`), and closes it after. A run that changed nothing leaves no entry (**to verify**: whether an empty record is elided by Rhino or must be dropped by the connector).
- **A failed run is undone.** If the script throws, is cancelled, or exceeds `max_duration_ms`, the executor closes the record and immediately calls `Undo()` on it, then reports what it reverted in `notices[]` (`script-rolled-back`, with the mutation counts). This is weaker than Revit's group rollback in two ways that `skill.md` states plainly: Rhino replays the record rather than discarding an uncommitted one, so a change that is not undoable (a `RunScript` command with side effects outside the document, a file written by the script) survives; and if the user acted between the script's last change and the rollback — impossible while the main thread is blocked in the run, but possible in the `pending` gap of a follow-up — `Undo()` would revert their action. The executor therefore verifies the top undo entry is its own record before calling `Undo()` and reports `script-rollback-skipped` if it is not, rather than reverting a person's work.
- **`Connector.Settle(doc, keep)`** closes the run's record early — required before `Save`/`SaveAs`/`Close` in the same run, as Revit, since those act on a document whose record is still open. Whether Rhino permits a save mid-record or silently splits the record is **to verify**.
...
- **Hard-blocked, no opt-in:** `RhinoDoc.BeginUndoRecord`/`EndUndoRecord`/`ClearUndoRecords`/`Undo`/`Redo` from a script (the connector owns the record; a script's own would split or destroy it), `RhinoApp.Exit`, `RhinoApp.RunScript` with a command token matching `_Exit`/`_Quit`, and — Rhino-specific — the interactive getters (§08). Refused at analysis time, `script-api-denied`.
...
Detection is a Python AST walk for Python and Roslyn's bound-symbol walk for C#, both keyed on *(type, member)* or *(command token)*, never bare names. `RunScript`'s argument is a string, so the gate for command tokens is a runtime check in the connector's `RunScript` wrapper as well as a static one on literal arguments — the static walk catches the common case cheaply and deterministically, the runtime check catches a computed command string, and both report the same code. The Revit rule that `public` means script-reachable in the bridge's assemblies applies in full, in both languages: CPython reaches every public .NET type through interop exactly as Roslyn does.
...
**Modal dialogs.** Rhino raises them through Eto on both platforms and through native Win32 boxes on Windows. There is no `DialogBoxShowing`-style pre-show event in RhinoCommon (**to verify** — `RhinoApp.CommandWindowCaptureEnabled` captures text, not dialogs). So v1 has no framework-level suppression; it has the Revit §07 v1 fallback from the start: on an `execute_script` timeout that leaves the run non-terminal, the plug-in enumerates the process's top-level windows from a background thread (Win32 `EnumWindows` on Windows; `NSApplication.windows` on macOS — a platform split the Revit connector never needed), returns titles and classes as diagnostic data, and takes no action. `Connector.DialogResultOverrides` exists so that when a suppressible hook is found it has a place to land; until then it is documented as no-op.
...
- **XML docs ship with the SDK, not necessarily with Rhino.** `RhinoCommon.xml` is in the NuGet package; whether the installed Rhino carries it is **to verify**. If not, the server embeds the doc sidecar per supported Rhino version, fetched at build time against a pin, as it already embeds the ranking models.
