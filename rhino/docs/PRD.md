...
| Cancellation | Cooperative only: `cancel.check()` in loops. **Verified** that `Rhino.Runtime.Code` exposes no interrupt for a running script, so a non-cooperating Python script resolves to `unrecoverable` after the grace period, as a non-cooperating C# one does | Cooperative `CancellationToken` as Revit |
...
- **One run, one Undo entry — by construction.** §06 runs every script inside one connector-owned command, so a run is one entry. A run that changed nothing leaves no entry (**verified** for an empty record). The entry's *name* is the command's name; the `MCP: <label>` naming Revit gives its entries needs a mechanism Rhino does not obviously offer (commands are registered statically) and is an open detail for phase 1, not a promise.
...
10. *(new)* The undo entry's name: whether a run can be named `MCP: <label>` when the entry takes the command's name (§07).
